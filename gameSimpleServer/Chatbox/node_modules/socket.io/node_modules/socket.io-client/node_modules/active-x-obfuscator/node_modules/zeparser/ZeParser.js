...
ZeParser.prototype = {
	...
	eatExpressions: function(mayParseLabeledStatementInstead, match, stack, onlyOne, forHeader, isBreakOrContinueArg){
		...
		do {
			...
			while (parseAnotherExpression) { // keep parsing lhs+operator as long as there is an operator after the lhs.
				...
				// object literals need seperate handling...
					...
					if (match.name == 2/*IDENTIFIER*/) {
						if (
							...
							// if break/continue, we skipped the unary operator check_ so throw the proper error here
					...
					// now check_ for labeled statement (if mayParseLabeledStatementInstead then the first token for this expression must be an (unreserved) identifier)
					if (mayParseLabeledStatementInstead && match.value == ':') {
						...
						// if no statement was found, check_ here now and correct error
				...
				// check_ for postfix operators ++ and --
				...
				} while (ternary); // if we just parsed a ternary expression, we need to check_ _again_ whether the next token is a binary operator.
	...
	eatObjectLiteralColonAndBody: function(match, stack){
		...
		// existing mechanisms. so we check_ this differently... :)
		var prevMatch = match;
	...
	eatLabelOrExpression: function(match, stack){
		...
		// in order to prevent very weird return constructs, we'll first check_ the first match
	...
	eatSourceElements: function(match, stack){
		...
		// the first argument indicates that the statement is optional. if that
		// statement was not found, the input match will also be the output.
		while (match != oldMatch) { // difficult to determine whether ` && match.name != 12/*EOF*/` is actually speeding things up. it's an extra check_ vs one less call to eatStatement...
			var oldMatch = match;
