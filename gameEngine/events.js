...
function event$dispatch( name ){
    ...
    var _args = collection$slice( arguments, 1 );
    
    var _array= event$_data[name];
    if( _array ){
...
function event$listen( name, fN ){
    ...
    if( !event$_data[name] ) event$_data[name] = [];
    event$_data[name].push( {fN:fN} ); // later we add timers too
}