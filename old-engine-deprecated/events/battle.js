...
model.event_on("attack_invoked", function (attId, defId, attLuckRatio, defLuckRatio) {
  ...
  var distance = model.map_getDistance(defender.x, defender.y, attacker.x, attacker.y);
  // counter attack when defender survives and defender is an indirect
  // attacking unit
  if (distance === 1 && /* retreatVal && */ defender.hp > 0 && !model.battle_isIndirectUnit(defId)) {
