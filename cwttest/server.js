//This is the prototype server for CWTactics
//Starting off with guest accounts
...
// Chat and Session Functions
// --------------------------

//This function starts the session for the client computer
function startSession(){
	var urlFrag = url + "?type=connect";
	httpRequest("GET", urlFrag, true, "");
}

// This function creates a user in the server
function createUser(){
	var code = document.getElementById("code");
	var user = document.getElementById("user");
	var urlFrag = url + "?type=add_user" +
...
function deleteUser(){
	var code = document.getElementById("code");
	var user = document.getElementById("user");
	var urlFrag = url + "?type=remove_user" +
		"&code=" + encodeURIComponent(code.value) +
		"&user=" + encodeURIComponent(user.value);
	httpRequest("GET", urlFrag, true, "");
}

//I'm writing the send message functionality for the server system
function sendMessage(){
	var code = document.getElementById("code");
	var user = document.getElementById("user");
	var message = document.getElementById("message");
	var urlFrag = url + "?type=send_chat" +
		"&code=" + encodeURIComponent(code.value) +
		"&user=" + encodeURIComponent(user.value) +
...
// When the window opens, get the token and the time it started
window.onload = function(){
	startSession();
	stored = new Date().getTime();
}

// When the user closes the session, delete the user from the database
window.onunload = function(){
	deleteUser();
}

// This is used to update the client with the most recent information
function pollServer(){
...
function handleResponse(){
	if(request.readyState == 4){
		if(request.status == 200){
			...
			if(objt.type == "connect"){
				//Gets the data from chat...
				var code = document.getElementById("code");
				code.value = objt.code;
			}else if(objt.type == "add_user"){
...
Parameters:
	...
	post: 'null' for GET requests, and 'text' for POST requests */
function initReq(reqType, url, isAsynch, post){
...
Parameters:
	...
	post: 'null' for GET requests, 'text' for POST requests */
function httpRequest(reqType, url, asynch, post){
	...
	if(request)
		initReq(reqType, url, asynch, post);
	else
		alert("Your browser does not permit the use of all this applications features!");
}