...
function storeImage(){
	...
	var tmpCut = cutArray.shift();
	if(tmpCut.length > 0){
		//This whole thing makes sure that the cut stays in bounds
		imgData = ctx.getImageData((tmpCut[0] > canvas.width-1) ? canvas.width-1 : tmpCut[0], 
			(tmpCut[1] > canvas.height-1) ? canvas.height-1 : tmpCut[1], tmpCut[2], tmpCut[3]);
		//and a proper length and width is stored
		imgWidth = (tmpCut[0] + tmpCut[2] > canvas.width) ? canvas.width - tmpCut[0] : tmpCut[2];
		imgHeight = (tmpCut[1] + tmpCut[3] > canvas.height) ? canvas.height - tmpCut[1] : tmpCut[3];
