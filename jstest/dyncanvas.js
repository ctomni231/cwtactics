 ...
 * Dynamic Canvas
 *
 * Eh, why not start a whole new class to understand the last few inner workings
 * of the canvas system. Here we will try to make a dynamic reacting canvas. As
 * well as answering the questions below:
 * 
 * - Make sure that the canvas reacts to the screen length and width
 * - Make this work for full screen display (alter the clicks)
 * - Understand how pinching works for fluid action for touch commands
 */
...
window.requestAnimFrame = (function (callback) {
	return window.requestAnimationFrame || 
		...
		function (callback) {
			window.setTimeout(callback, second);
		};
})();
...
window.addEventListener("resize", function(e) {
	  ...
	  if(w > h) {
		imgStorage.setAttribute("width", 800);
		imgStorage.setAttribute("height", 450);
	  }
	  else {
		imgStorage.setAttribute("width", 450);
		imgStorage.setAttribute("height", 800);
	  }
});
