...
util.scoped(function() {
    ...
    util.grabRemoteFile = function(options) {
        ...
        oReq.open("get", options.path + "?_cwtR=" + parseInt(1e4 * Math.random(), 10), true);
        oReq.send();
    };
});
...
util.scoped(function() {
    ...
    var animations = [ 0, 3, 0, 250, 0, 0, 3, 0, 250, 0, 0, 7, 0, 150, 0, 0, 20, 0, 300, 0, 0, 4, 0, 400, 0, 0, 4, 0, 500, 0, 0, 8, 0, 500, 0 ];
    var unitStepper = +1;
...
controller.ButtonGroup = {
    ...
    decreaseIndex: function(amount) {
        if (amount === void 0) amount = -1; else amount = -amount;
        this.changeIndex(amount, true);
    },
...
controller.coMusic_playLoadedMusic_ = function(_, id) {
    ...
    if (view.hasInfoMessage()) view.message_closePanel(1e3);
};

controller.coMusic_storeAudio_ = function(obj) {
    var audioData = Base64Helper.decodeBuffer(obj.value);
    controller.audio_loadByArrayBuffer(obj.key, audioData, controller.coMusic_playLoadedMusic_);
};
...
controller.setCursorPosition = function(x, y, relativeToScreen, preventSound) {
    ...
    if (state === "ACTION_SELECT_TARGET_A") {
        ...
        if (data.selection.getValueAt(x, y) > 0) {
            ...
            if (targetUnit) {
                dmg = model.battle_getBattleDamageAgainst(data.source.unit, targetUnit, 0, model.battle_canUseMainWeapon(data.source.unit, targetUnit), false, data.source.x, data.source.y);
            }
        }
    }
...
view.imageProcessor_cropMiscSprite = function(miscType) {
    if (miscType.length > 2) {
        ...
        if (miscType.length > 6 && miscType[6] === true) {
            ...
            nContext.drawImage(img, miscType[2], miscType[3], miscType[4], miscType[5], 0, 0, 24, 24);
        } else {
...
util.scoped(function() {
    ...
    controller.registerMenuRenderer = function(key, renderer) {
        assert(!menuRenderer.hasOwnProperty(key), "renderer for", key, "is already registered");
    ...
    var selectPage = function(page, first) {
        ...
        for (var i = 0, e = menuSize; i < e; i++) {
            if (relIndex + i < menu.size) {
                ...
                renderer(menu.data[relIndex + i], entries[i].children[0], relIndex + i, menu.enabled[relIndex + i]);
            } else {
                entries[i].style.display = "none";
            }
        }
        if (first) controller.setMenuIndex(0); else controller.setMenuIndex(menuSize - 1);
        menuPage = page;
    };
    ...
    controller.increaseMenuCursor = function() {
        if (controller.menuCursorIndex < menuSize - 1 && controller.menu_getSelectedIndex() < controller.stateMachine.data.menu.size - 1) {
...
controller.metadata_grabFromMapData = function(mapData, btns, canvases, types) {
    ...
    for (var i = 0; i < btns.length; i++) {
        ...
        if (mapData === null || types === null || i >= types.length) {
            ...
            btns[i].innerHTML = n;
            ctx.drawImage(view.getPropertyImageForType(type, view.COLOR_NEUTRAL), 0, 0, 16, 32, 0, 0, 16, 32);
        }
    }
};
...
controller.minimap_showIngameMinimap = function() {
    ...
    var cv = cvEl.style;
    controller.minimap_renderMinimap_(cvEl, model.map_width, model.map_height, model.map_data, null, true);
    blend.display = "block";
    cv.display = "block";
    ...
    for (x = 0; x < xe; x++) {
        for (y = 0; y < ye; y++) {
            ...
            if (type.assets.mmap4x_gfx !== void 0) {
                ctx.drawImage(img, type.assets.mmap4x_gfx * base, 0, base, base, x * base, y * base, base, base);
            } else {
...
controller.minimap_renderMapSelectionMinimap = function(mapData) {
    ...
    controller.minimap_renderMinimap_(cv, mapData.mpw, mapData.mph, mapData.map, mapData.typeMap, mapData.mph <= MAX_MAP_HEIGHT / 2 && mapData.mpw <= MAX_MAP_WIDTH);
...
controller.network_parseMessage = function() {
    ...
    return function() {
        ...
        xmlHttp.open("GET", controller.network_target + "?cmd=GRABCMD&gameId=" + controller.network_gameId + "&userId=" + controller.network_clientId, true);
...
view.renderMap = function(scale) {
    ...
    var focusExists = controller.stateMachine.state === "MOVEPATH_SELECTION" || controller.stateMachine.state === "ACTION_SELECT_TARGET_A" || controller.stateMachine.state === "ACTION_SELECT_TARGET_B" || controller.attackRangeVisible;
    var movePathVisible = controller.stateMachine.state === "MOVEPATH_SELECTION" || controller.stateMachine.state === "ACTION_MENU" || controller.stateMachine.state === "ACTION_SUBMENU";
...
controller.dataLoader_start = function(loadDescComponent, loadBarComponent) {
    ...
    jWorkflow.order().andThen(function() {
        ...
        controller.storage_general.get("cwt_animatedTiles", function(obj) {
            if (obj) {
                if (obj.value === true) controller.screenStateMachine.structure.OPTIONS.animatedTiles = true; else controller.screenStateMachine.structure.OPTIONS.animatedTiles = false;
    ...
    }).andThen(controller.loadImages_doIt).andThen(function(p, b) {
        b.take();
    ...
    controller.updateGamePadControls = function(canvas, menuEl) {
        ...
        for (var i = 0, e = 4; i < e; i++) {
            ...
            if (controller.input_genericInputRequest && controller.screenStateMachine.state === "REMAP_GAMEPAD") {
                ...
                var key = null;
                if (gamepad.axes[1] < -.5) key = keymapAct.UP; else if (gamepad.axes[1] > +.5) key = keymapAct.DOWN;
                if (gamepad.axes[0] < -.5) key = keymapAct.LEFT; else if (gamepad.axes[0] > +.5) key = keymapAct.RIGHT;
                if (gamepad.buttons[keymap.ACTION] === 1) key = keymapAct.ACTION; else if (gamepad.buttons[keymap.CANCEL] === 1) key = keymapAct.CANCEL;
    ...
    controller.setupKeyboardControls = function(canvas, menuEl) {
        ...
        document.onkeydown = function(ev) {
            ...
            if (controller.input_genericInputRequest && controller.screenStateMachine.state === "REMAP_KEYBOARD") {
                ...
                switch (ev.keyCode) {
                  case keymap.LEFT:
                    key = controller.DEFAULT_KEY_MAP.KEYBOARD.LEFT;
                    break;

                  case keymap.UP:
                    key = controller.DEFAULT_KEY_MAP.KEYBOARD.UP;
                    break;

                  case keymap.RIGHT:
                    key = controller.DEFAULT_KEY_MAP.KEYBOARD.RIGHT;
                    break;

                  case keymap.DOWN:
                    key = controller.DEFAULT_KEY_MAP.KEYBOARD.DOWN;
                    break;

                  case keymap.CANCEL:
                    key = controller.DEFAULT_KEY_MAP.KEYBOARD.CANCEL;
                    break;

                  case keymap.ACTION:
                    key = controller.DEFAULT_KEY_MAP.KEYBOARD.ACTION;
                    break;
                }
            }
...
controller.setupTouchControls = function(canvas, menuEl) {
    util.scoped(function() {
        function oneFingerTap(event, x, y) {
            ...
            if (!controller.menuVisible) {
                ...
                if (event.target.id === "cwt_menu") {
                    controller.input_pushKey(controller.keyMaps.KEYBOARD.ACTION, INACTIVE_ID, INACTIVE_ID);
                } else {
                    controller.input_pushKey(controller.keyMaps.KEYBOARD.CANCEL, INACTIVE_ID, INACTIVE_ID);
                }
            }
        }
        ...
        function swipe(event, dx, dy) {
            ...
            controller.input_pushKey(key, controller.screenStateMachine.state === "GAME_ROUND" ? 10 : 1, INACTIVE_ID);
        }
...
controller.registerMenuRenderer("buildUnit", function(content, entry, index, enabled) {
    ...
    if (enabled) entry.innerHTML = model.data_localized(content) + " (" + cost + " G)"; else entry.innerHTML = "<span style='color:red;'>" + model.data_localized(content) + " (" + cost + " G) </span>";
});
    ...
    prepare: function(prid, cid) {
        ...
        if (model.fog_clientData[property.x][property.y] > 0) {
            if (property.capturePoints === 20) {
                ...
                view.showInfoMessage(model.data_localized("propertyPointsLeft") + " " + property.capturePoints);
            }
        }
    },
    render: function() {},
...
view.registerAnimationHook({
    ...
    prepare: function(uid, x, y) {
        ...
        this.moveAnimationClientOwned = model.fog_visibleClientPids[model.unit_data[uid].owner];
...
util.scoped(function() {
    view.registerAnimationHook({
        ...
        prepare: function() {
            view.showInfoMessage(model.data_localized("day") + " " + model.round_day + " - " + model.player_data[model.round_turnOwner].name, 999999999);
            if (controller.features_client.audioMusic) {
                var co = model.co_data[model.round_turnOwner].coA;
                controller.coMusic_playCoMusic(co ? co.music : null);
            } else view.message_closePanel(1e3);
        },
        render: function() {},
        update: function(delta) {},
        isDone: function() {
...
util.scoped(function() {
    ...
    var btn = controller.generateButtonGroup(document.getElementById("cwt_confirmWipeOut_screen"), "cwt_panel_header_small cwt_page_button w_400 cwt_panel_button", "cwt_panel_header_small cwt_page_button w_400 cwt_panel_button button_active", "cwt_panel_header_small cwt_page_button w_400 cwt_panel_button button_inactive");
...
util.scoped(function() {
    ...
    controller.screenStateMachine.structure.GAMEROUND.CANCEL = function(ev, x, y) {
        if (!controller.menuVisible) {
            ...
            if (controller.stateMachine.state === "IDLE" && (!model.fog_turnOwnerData[mx][my] || !prop && !unit)) {
...
controller.screenStateMachine.structure.NONE.start = function() {
    ...
    (function setupAnimationFrame() {
        ...
        function looper() {
            ...
            oldTime = now;
            if (delta > 32) {
                if (evenFrame) drops++; else drops2++;
            ...
            if (controller.inGameLoop) {
                ...
                } else controller.screenStateMachine.event("gameHasEnded");
            }
            lastDelta = delta;
...
util.scoped(function() {
    ...
    function updateAnimatedTilesContent() {
        nodeAnimTiles.innerHTML = controller.screenStateMachine.structure.OPTIONS.animatedTiles ? model.data_localized("yes") : model.data_localized("no");
    }
...
util.scoped(function() {
    var btn = controller.generateButtonGroup(document.getElementById("cwt_parameter_setup_screen"), "cwt_panel_header_big cwt_page_button cwt_panel_button", "cwt_panel_header_big cwt_page_button cwt_panel_button button_active", "cwt_panel_header_big cwt_page_button cwt_panel_button button_inactive");
    var cPage = 0;
    var parameters = [ null, null, null, null, null ];
    var parameterBtn = [ document.getElementById("options.parameter.1.value"), document.getElementById("options.parameter.2.value"), document.getElementById("options.parameter.3.value"), document.getElementById("options.parameter.4.value"), document.getElementById("options.parameter.5.value") ];
    var parameterTextBtn = [ document.getElementById("options.parameter.1.desc"), document.getElementById("options.parameter.2.desc"), document.getElementById("options.parameter.3.desc"), document.getElementById("options.parameter.4.desc"), document.getElementById("options.parameter.5.desc") ];
...
util.scoped(function() {
    ...
    function updateGamemodeBtn() {
        var key = null;
        if (model.co_activeMode === model.co_MODES.AW1) key = "options.playerConf.mode.AW1"; else if (model.co_activeMode === model.co_MODES.AW2) key = "options.playerConf.mode.AW2";
    ...
    function updateButtonsForPlayer(pid) {
        ...
        if (type === INACTIVE_ID) updateButtons("config.player.off", "&#160;", -1); else if (type === DESELECT_ID) updateButtons("config.player.disabled", "&#160;", -1); else updateButtons(type === 0 ? "config.player.human" : "config.player.AI", controller.roundConfig_coSelected[pid] !== INACTIVE_ID ? model.data_coSheets[model.data_coTypes[controller.roundConfig_coSelected[pid]]].ID : "config.player.co.none", controller.roundConfig_teamSelected[pid]);
...
(function() {
    ...
    var btn = controller.generateButtonGroup(document.getElementById("cwt_keyMapping_screen"), "cwt_panel_header_small cwt_page_button w_400 cwt_panel_button", "cwt_panel_header_small cwt_page_button w_400 cwt_panel_button button_active", "cwt_panel_header_small cwt_page_button w_400 cwt_panel_button button_inactive");
...
util.scoped(function() {
    ...
    var cPage = 0;
    var cCategory = 0;
    var nameBtns = [ document.getElementById("options.versus.map.1"), document.getElementById("options.versus.map.2"), document.getElementById("options.versus.map.3"), document.getElementById("options.versus.map.4"), document.getElementById("options.versus.map.5") ];
    var metaBtns = [ document.getElementById("options.versus.meta.1"), document.getElementById("options.versus.meta.2"), document.getElementById("options.versus.meta.3"), document.getElementById("options.versus.meta.4"), document.getElementById("options.versus.meta.5"), document.getElementById("options.versus.meta.6") ];
    var metaCanvasBtns = [ document.getElementById("options.versus.meta.1.canvas"), document.getElementById("options.versus.meta.2.canvas"), document.getElementById("options.versus.meta.3.canvas"), document.getElementById("options.versus.meta.4.canvas"), document.getElementById("options.versus.meta.5.canvas"), document.getElementById("options.versus.meta.6.canvas") ];
    var nameValues = [ null, null, null, null, null ];
    ...
    function mapLoadFinish(obj) {
        controller.metadata_grabFromMapData(obj.value, metaBtns, metaCanvasBtns, model.data_header.map_meta);
