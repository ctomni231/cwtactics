...
controller.actionBuilder_buildFromUserData = function(scope) {
    ...
    if (!trapped) actionObject.invoke(scope); else controller.commandStack_sharedInvokement("trapwait_invoked", sourceDto.unitId);
...
controller.roundConfig_changeConfig = function(pid, type, prev) {
    ...
    switch (type) {
      ...
      case controller.roundConfig_CHANGE_TYPE.GAME_TYPE:
        if (prev) {
            if (model.co_activeMode === model.co_MODES.AW1) model.co_activeMode = model.co_MODES.AW2; else if (model.co_activeMode === model.co_MODES.AW2) model.co_activeMode = model.co_MODES.AW1;
        } else {
            if (model.co_activeMode === model.co_MODES.AW1) model.co_activeMode = model.co_MODES.AW2; else if (model.co_activeMode === model.co_MODES.AW2) model.co_activeMode = model.co_MODES.AW1;
        }
        break;
...
model.battle_getBattleDamageAgainst = function(attacker, defender, luck, withMainWp, isCounter, ax, ay) {
    ...
    controller.prepareTags(ax, ay, model.unit_extractId(attacker), defender.x, defender.y, model.unit_extractId(defender));
...
model.co_data = util.list(MAX_PLAYER, function(i) {
    return {
        power: 0,
        timesUsed: 0,
        level: model.co_POWER_LEVEL.INACTIVE,
        coA: null,
        coB: null,
...
model.data_movetypeParser = model.data_createParser("movetype", model.data_movetypeSheets, function(sheet) {
...
model.event_on("activatePower_invoked", function(pid, level) {
    ...
    assert(coData.coA);
    var powerData = level === model.co_POWER_LEVEL.COP ? coData.coA.cop.power : coData.coA.scop.power;
    if (powerData.healUnits) {
        model.events.copower_healUnits(pid, controller.scriptedValueByRules(powerData.healUnits, pid, "target", 0) + model.player_RELATION_MODES.OWN, controller.scriptedValueByRules(powerData.healUnits, pid, "amount", 0));
    }
    if (powerData.damageUnits) {
        model.events.copower_damageUnits(pid, controller.scriptedValueByRules(powerData.damageUnits, pid, "target", 0) + model.player_RELATION_MODES.OWN, controller.scriptedValueByRules(powerData.damageUnits, pid, "amount", 0));
    } else if (powerData.copower_changeWeather) {
        model.events.copower_healUnits(controller.scriptedValueByRules(powerData.healUnits, pid, "weather", 0), controller.scriptedValueByRules(powerData.healUnits, pid, "days", 1));
    }
});
...
model.event_on("modifyVisionAt", function(x, y, pid, range, value) {
    ...
    if (range === 0) {
        ...
        for (;lY <= hY; lY++) {
            ...
            if (hX >= mW) hX = mW - 1;
            for (;lX <= hX; lX++) {
                if (model.map_data[lX][lY].blocksVision && model.map_getDistance(x, y, lX, lY) > 1) continue;
...
model.event_on("move_flushMoveData", function(move, source) {
    ...
    for (var i = 0, e = move.length; i < e; i += 6) {
        ...
        controller.commandStack_sharedInvokement("move_appendToWayCache", move[i], move[i + 1], move[i + 2], move[i + 3], move[i + 4], move[i + 5]);
    }
    controller.commandStack_sharedInvokement("move_moveByCache", source.unitId, source.x, source.y, 0);
});
...
controller.action_unitAction({
    ...
    condition: function(data) {
        return model.events.attack_check(data.source.unitId, data.target.x, data.target.y, data.movePath.data[0] !== INACTIVE_ID);
    },
    invoke: function(data) {
...
controller.action_mapAction({
    ...
    invoke: function(data) {
        controller.commandStack_sharedInvokement("transferMoney_invoked", model.round_turnOwner, data.target.unit ? data.target.unit.owner : data.target.property.owner, data.action.selectedSubEntry);
    }
});
...
(function() {
    ...
    controller.ai_defineRoutine({
        ...
        scoring: function(data, cScore) {
            ...
            if (cprop && (cprop.owner === INACTIVE_ID || model.player_data[cprop.owner].team !== tid) && model.property_isCapturableBy(model.property_extractId(cprop), data.source.unitId)) {
                prop = cprop;
        ...
        prepare: function(data) {
            ...
            if (!trapped) {
                ...
                controller.commandStack_sharedInvokement("trapwait_invoked", data.source.unitId);
            }
        }
    });
})();
...
(function() {
    ...
    controller.ai_defineRoutine({
        ...
        scoring: function(data, cScore) {
            ...
            if (!data.source.unit.type.attack) return -1;
            if (!model.events.attack_check(data.source.unitId, data.source.x, data.source.y, false)) return -1;
            model.battle_calculateTargets(data.source.unitId, data.source.x, data.source.y, data.selection, false);
            data.selection.nextRandomPosition(setTarget, data, 1);
            if (data.targetselection.unitId === -1) return -1;
            return 10;
        },
    ...
    controller.ai_defineRoutine({
        ...
        scoring: function(data, cScore) {
            ...
            while (true) {
                ...
                if (!data.target.unit) break; else n++;
            }
            model.move_generatePath(data.source.x, data.source.y, data.target.x, data.target.y, data.selection, data.move);
