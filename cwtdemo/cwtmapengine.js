...
import * as cwtmaplib from "./tools/maplibrary.js"
import * as cwtmaptest from "./tools/maplib.js"
...
// Work on getting the loaded map to display
// Then work on the menu
// Then work on the fog types
// MOW FOW (AW2 and AW4) / Detected / DARK

export const name = "CWTMAP"

// Let's define all the colors in a single function
export const cwtcolor = {
  tile: {},
  team: [],
...
export function init(){
  ...
  // Holds the various tile colors used in the map
  cwtcolor.tile["move"] = "#007F7F";
  cwtcolor.tile["fire"] = "red";

  // Holds the various terrain colors used in the map
  cwtcolor.terr["PLIN"] = "#B6FF00";
  cwtcolor.terr["NULL"] = "gray";

  // Holds the various property colors used in the map
  cwtcolor.prop["GD"] = "gray";
  cwtcolor.prop["OS"] = "red";
  cwtcolor.prop["BM"] = "blue";
  cwtcolor.prop["YC"] = "yellow";
  cwtcolor.prop["GE"] = "green";
  cwtcolor.prop["BH"] = "purple";

  // Holds the various unit colors used in the map
  cwtcolor.unit["GD"] = "gray";
  cwtcolor.unit["OS"] = "red";
  cwtcolor.unit["BM"] = "blue";
  cwtcolor.unit["YC"] = "yellow";
  cwtcolor.unit["GE"] = "green";
  cwtcolor.unit["BH"] = "purple";

  // Lists the various team colors used in the map
  cwtcolor.team = (cwtcolor.unit.length > cwtcolor.prop.length) ? getKeys(cwtcolor.unit) : getKeys(cwtcolor.prop);
}
...
export function getTerrainColor(key){
  return getColor(key, cwtcolor.terr);
}

export function getPropertyColor(key){
  return getColor(key, cwtcolor.prop);
}

export function getUnitColor(key){
  return getColor(key, cwtcolor.unit);
}

export function getDefaultColor(key){
  return getColor(key, cwtcolor.dflt);
}

function getColor(key, maindict, dfltcolor='gray') {

  let value = undefined

  // This gets a terrain off of a string key
  ...
  // This uses the index to find a terrain
  if ( typeof key === 'number'){
    let dictarr = getKeys(maindict)
    if (dictarr.length > 0){
      let tmpkey = (key > 0 && key < dictarr.length) ? key : 0;
      value = maindict[dictarr[tmpkey]]
    }
  }

  // This will return some kind of default value
  if (value === undefined)
    value = dfltcolor
...
// For getting just the keys out of a dictionary
function getKeys(maindict){
  let tmparr = []
...
// Tells you if we have a map in the system
export function isMap() {
  return (cwtmap.sizex > 0 && cwtmap.sizey > 0)
}
...
export function createFileMap(ind, game="CWT"){

  // Let's just turn a string into a list
  if ( typeof key === 'string')
    game = [game]

  // First, let's check if everything is loaded into the system
  if (cwtmaplib.isAllMapDataReady()){

    let data = cwtmaplib.getMapData(ind);

    let text = "";
    for (const x in data) {
      text += x + ", ";
    }
    //console.log(text, data[game[0]]);

    cwtmap.sizex = 1
    cwtmap.sizey = 1

    // Let's see if we can create terrain packs
    cwtmaplib.createTerrainPacks(game)
    console.log("TERRDICT", cwtmaplib.getTerrainDict(), cwtmaplib.getTerrainPackData())
    cwtmaplib.packTerrainText()
    console.log("TERRTEXT", cwtmaplib.getTerrainTextData(), cwtmaplib.getTerrainTextDict())

    console.log("PLIN", cwtmaplib.getTerrainDataKey("PLIN"))
    console.log("NULL", cwtmaplib.getTerrainDataKey("NULL"))

    console.log("NULL", cwtmaplib.getTerrainTextKey("NULL","name"))
    console.log("PLIN", cwtmaplib.getTerrainTextKey("PLIN","description"))
  }
}//*/
...
export function createNullBorderMap(sizex, sizey){
...
function drawMap(ctx){
  ...
  for(let i=0; i<cwtmap.tid.length; i++){
    ...
    intcolor = getTerrainColor(cwtmap.tid[i])
    if (cwtmap.mv[i] > 0) // Movement
      intcolor = cwtcolor.tile["move"]
    if (cwtmap.atk[i] > 0) // Attack
      intcolor = cwtcolor.tile["fire"]
    ...
    drawPolyVerts(ctx, px, py, intcolor)
  }
}

function drawUnit(ctx, owner=0, fillcolor='', strokecolor='black'){
  ...
  for(let i=0; i<cwtunit.uid.length; i++){
    ...
    if (fillcolor != "")
      ...
      if (owner < 0 || owner >= cwtunit.color.length)
        owner = 0
      tmpcolor = cwtunit.color[owner] // The eventual owner number
    }
