...
public class InputHandlerBean {
	...
	public InputHandlerBean() {

		KEYBOARD_MAPPING = JSCollections.$map();
		KEYBOARD_MAPPING.$put(InputType.UP.name(), 38);
		KEYBOARD_MAPPING.$put(InputType.DOWN.name(), 40);
		KEYBOARD_MAPPING.$put(InputType.LEFT.name(), 37);
		KEYBOARD_MAPPING.$put(InputType.RIGHT.name(), 39);
		KEYBOARD_MAPPING.$put(InputType.ACTION.name(), 13);
		KEYBOARD_MAPPING.$put(InputType.CANCEL.name(), 8);

		GAMEPAD_MAPPING = JSCollections.$map();
		GAMEPAD_MAPPING.$put(InputType.ACTION.name(), 0);
		GAMEPAD_MAPPING.$put(InputType.CANCEL.name(), 1);
