...
cwt.Factory = {
  ...
  buildUnit: function (factory, type) {
    if (this.DEBUG) assert(factory instanceof cwt.Property);
    if (this.DEBUG) assert(cwt.UnitSheet.isValidSheet(type));
    ...
    if (this.DEBUG) assert(factory.owner.gold >= 0);
  ...
  generateBuildMenu: function (factory, menu, markDisabled) {
    if (this.DEBUG) assert(factory instanceof cwt.Property);
    if (this.DEBUG) assert(menu instanceof cwt.Menu);
    if (this.DEBUG) assert(factory.owner);
