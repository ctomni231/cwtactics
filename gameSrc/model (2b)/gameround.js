...
cwt.Gameround = {
   ...
   * list with `model (2b).actions_markUnitNonActable`.
   */
  actors: [],
  ...
  playerStartsTurn_: function (player) {
    ...
    /*
     ...
     var check = model (2b).unit_thereIsAUnit;
     var mode = model (2b).MODE_OWN;
     if (controller (3).configValue("supplyAlliedUnits") === 1) mode = model (2b).MODE_TEAM;

     if (check(x, y, pid, mode)) {
     var unitTp = model (2b).unit_posData[x][y].type;
     if (prop.type.supply.indexOf(unitTp.ID) !== -1 ||
     prop.type.supply.indexOf(unitTp.movetype) !== -1) {

     model (2b).events.supply_refillResources(model (2b).unit_posData[x][y]);
     }
     }
     }
     */
    ...
    /*
     ...
     var check = model (2b).unit_thereIsAUnit;
     var mode = model (2b).MODE_OWN;
     if (controller (3).configValue("repairAlliedUnits") === 1) mode = model (2b).MODE_TEAM;
     ...
     // script it :P
     value = controller (3).scriptedValue(pid, "propertyHeal", value);

     if (value > 0) {
     model (2b).events.healUnit(
     model (2b).unit_extractId(model (2b).unit_posData[x][y]),
     model (2b).unit_convertPointsToHealth(value),
     true
     );
     }
     }
  ...
  calculateNextWeather: function (wth) {
    ...
    var newTp;
    var duration;
    if (this.weather && this.weather.defaultWeather) {
