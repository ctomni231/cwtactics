...
public class Position {
  ...
  /**
   * Sets a position. public void set(int x, int y) { this.clean();
   * 
   * this.x = x; this.y = y; this.tile = net.wolfTec.gameround.map.getTile(x,
   * y);
   * 
   * if (tile.visionTurnOwner > 0 && this.tile.unit != null) { this.unit =
   * tile.unit; this.unitId = net.wolfTec.gameround.units.indexOf(tile.unit); }
   * 
   * if (tile.property != null) { property = tile.property; propertyId =
   * CustomWarsTactics.gameround.getPropertyId(tile.property); } }
   *
   * @param x
   * @param y
   */
