...
public class MoveType extends ObjectType {
  ...
  protected void validateData(Array<String> errors) {
    JsUtil.forEachObjectValue(costs, (tileTypeId, movecosts) -> {
      checkExpression(Is.is.string(tileTypeId), errors, "costs -> " + tileTypeId + " key");
      // TODO id exists check + is tile check
      checkExpression(Is.is.integer((Number) movecosts) && Is.is.within((Number) movecosts, -1, 100) && Is.is.not.equal((Number) movecosts, 0), errors, "costs -> " + tileTypeId + " value");
    });
  }
}
