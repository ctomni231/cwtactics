...
/* pre-generate an in-memory menu object that can be used as ... */
    ...
    for (int i = 0, e = path.getSize(); i < e; i++) {
      ...
      if (tX == -1) {
        ...
        if (diffX == 2) {
          ...
          Debug.logCritical(LOG_HEADER, "illegal move arrow state -> old (" + oX + "," + oY
              + ") -> current (" + x + "," + y + ") -> next (" + tX + "," + tY + ") -> path ("
              + path + ")");
          continue;
        }
      }
      ...
      if (x >= 0 && y >= 0) {
        layerUI.getContext(0).drawImage(pic, (x - offsetX) * Constants.TILE_BASE,
  ...
  public void prepareMenu() {
    ...
    layoutGenericMenu.setMenuPosition(parseInt((screenWidth / 2) - MENU_ENTRY_WIDTH / 2, 10),
    ...
    for (var i = 0; i < MENU_ELEMENTS_MAX; i++) {
      if (i < numElements) {
        ...
        // set style
        gfxMenu.elements[i].style = ((numElements == 1 ? widgets.UIField.STYLE_NORMAL
            : (i == 0 ? widgets.UIField.STYLE_NEW : (i == numElements - 1
                || i == menuShift + MENU_ELEMENTS_MAX - 1 ? widgets.UIField.STYLE_ESW
  ...
  public void handleMenuInput(MoveCode code) {
    ...
    if (menu.getSize() > MENU_ELEMENTS_MAX) {
      ...
      switch (code) {
        ...
        case DOWN:
          if (currentIndex > MENU_ELEMENTS_MAX - 3
              && menuShift < menu.getSize() - MENU_ELEMENTS_MAX) {
            shiftedMenu = true;
            menuShift++;
          } else if (currentIndex == MENU_ELEMENTS_MAX - 1
              && menuShift == menu.getSize() - MENU_ELEMENTS_MAX) {
            shiftedMenu = true;
            menuShift = 0;
