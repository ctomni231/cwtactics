...
public class ConfigurationPeristence {
  ...
  private void grabConfigFromUrl(String paramName, TagValue cfg) {
  ...
  private void saveConfig(Map<String, Integer> data, TagValue cfg) {
    cfg.setValue(NullUtil.getOrElse(data.$get(cfg.key), cfg.def));
  }

  private void loadConfig(Map<String, Integer> data, TagValue cfg) {
    data.$put(cfg.key, cfg.value);
  }

  private boolean isAppCfg(TagValue cfg) {
