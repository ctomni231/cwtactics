...
public class TransportLogic implements ManagedClass
{
  ...
  public boolean canLoadUnit(Unit transporter, Unit load)
  {
    return (transporter.type.canload.indexOf(load.type.movetype) != -1
        || transporter.type.canload.indexOf(load.type.ID) != -1);
  }
  ...
  public boolean canUnloadSomethingAt(Unit transporter, int x, int y)
  {
    ...
    for (int i = 0, e = Constants.MAX_UNITS * Constants.MAX_PLAYER; i < e; i++)
    {
      ...
      if (unit.loadedIn == tid)
      {
        ...
        if (model.isValidPosition(x - 1, y) && move.canTypeMoveTo(moveType, x - 1, y))
          return true;
        if (model.isValidPosition(x + 1, y) && move.canTypeMoveTo(moveType, x + 1, y))
          return true;
        if (model.isValidPosition(x, y - 1) && move.canTypeMoveTo(moveType, x, y - 1))
          return true;
        if (model.isValidPosition(x, y + 1) && move.canTypeMoveTo(moveType, x, y + 1))
          return true;
      }
