 ...
 */
  ...
  public void loadVolumeConfigs(Callback0 callback) {
    pm.get(SFX_VOLUME_KEY, (err, value) -> {
      Nullable.ifPresent(value, () -> audio.setSfxVolume(value));

      pm.get(MUSIC_VOLUME_KEY, (merr, mvalue) -> {
        Nullable.ifPresent(mvalue, () -> audio.setMusicVolume(mvalue));
        Nullable.ifPresent(callback, () -> callback.$invoke());
    });
  }
}
