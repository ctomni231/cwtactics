...
public class StateDataSelection {
  ...
  @Template("toProperty") public Object getData() {
    return this.data;
  }

  @Template("toProperty") public int getCenterX() {
    return this.centerX;
  }

  @Template("toProperty") public int getCenterY() {
    return this.centerY;
  }
  ...
  public boolean hasActiveNeighbour(int x, int y) {
    ...
    return ((x > 0 && this.data.$get(x - 1).$get(y) > 0)
        ...
        || (x < SIDE_LENGTH - 1 && this.data.$get(x + 1).$get(y) > 0) || (y < SIDE_LENGTH - 1 && this.data
        .$get(x).$get(y + 1) > 0));
  }

  public void nextValidPosition(int x, int y, int minValue, boolean walkLeft,
      Callback3<Integer, Integer, Object> cb, Object arg) {
    x = x - this.centerX;
    y = y - this.centerY;
   ...
   * with a value greater equals minValue and lower equals maxValue.
   *
   * @param minValue
   * @param maxValue
   */
  public void onAllValidPositions(int minValue, int maxValue,
      Callback4<Integer, Integer, Integer, Map<String, Object>> cb, Map<String, Object> args) {
    for (int x = 0; x < SIDE_LENGTH; x++) {
      for (int y = 0; y < SIDE_LENGTH; y++) {
