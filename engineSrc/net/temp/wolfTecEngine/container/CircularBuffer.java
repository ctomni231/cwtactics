...
public class CircularBuffer<T> {
  ...
  public CircularBuffer(int maxSize) {
    if (maxSize <= 0) JsUtil.raiseError("size cannot be 0 or lower");
  ...
  public T get(int index) {
    if (index < 0 || index >= this.size) JsUtil.raiseError("illegal index");
   ...
   */
  public T popFirst() {
    if (this.size == 0) JsUtil.raiseError("buffer is empty");
  ...
  public void push(T el) {
    if (this.size == this.maxSize) JsUtil.raiseError("buffer is full");
  ...
  public static <M> void copyBuffer(CircularBuffer<M> source, CircularBuffer<M> target) {
    if (target.maxSize != source.maxSize) JsUtil.raiseError("same size required");
