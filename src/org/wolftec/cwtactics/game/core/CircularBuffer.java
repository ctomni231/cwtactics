...
public class CircularBuffer<T> {
  ...
  public CircularBuffer(int lMaxSize) {
    if (lMaxSize <= 0) raiseError("size cannot be 0 or lower");
  ...
  public T get(int index) {
    if (index < 0 || index >= this.size) raiseError("illegal index");
  ...
  public void add(T el) {
    if (this.size == this.maxSize) raiseError("buffer is full");
  ...
  public static <M> void copyBuffer(CircularBuffer<M> source, CircularBuffer<M> target) {
    if (target.maxSize != source.maxSize) raiseError("same size required");
  ...
  private static void raiseError(String error) {
    JSObjectAdapter.$js("throw new Error(error)");
  }
}
