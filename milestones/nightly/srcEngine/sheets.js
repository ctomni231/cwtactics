cwt.defineLayer( CWT_LAYER_SHEETS, function( sheets ){

  var dbAmanda = amanda("json");
  ...
  sheets.parseSheet = function( data, type ){
    ...
    // VALIDATE SHEET
    dbAmanda.validate( data, schema, function(e){
...
cwt.defineLayer( CWT_LAYER_DATA_ACCESS, function( data, model, sheets, util ){
   ...
   * @param id
   */
  data.movetypeSheet = function(id){
   ...
   *
   */
  data.getListOfUnitTypes = function(){
   ...
   *
   */
  data.getListOfPropertyTypes = function(){
   ...
   *
   */
  data.getListOfTileTypes = function(){
    var l = Object.keys( sheets.tiles );
    var r = [];
    for( var i=l.length-1; i>=0; i-- ){
