const cwtCreateScreenHandler = function () {
  ...
  screens.START = {
    ...
    onenter(transition, data) {
      gameClient.jobs.add("start:tooltip:update", 5000, () =>
        data.activeTooltip = gameClient.translated("tooltip." + parseInt(Math.random() * 10, 10) + 1))
    },
    actions: {
  ...
  screens.OPTIONS = {
    ...
    actions: {
      decreaseSFX: (transition, data) => data.sfxVolume = gameClient.setSfxVolume(data.sfxVolume - 1),
      increaseSFX: (transition, data) => data.sfxVolume = gameClient.setSfxVolume(data.sfxVolume + 1),
      decreaseMusic: (transition, data) => data.musicVolume = gameClient.setMusicVolume(data.musicVolume - 1),
      increaseMusic: (transition, data) => data.musicVolume = gameClient.setMusicVolume(data.musicVolume + 1),
  ...
  screens.INGAME = {
    ...
    onenter() {
      ...
      gameClient.jobs.add("inGameLoop", 100, delta => exports.actions.handleNextCommand(world))
    },
