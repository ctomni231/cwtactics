const gameCreateActionHandler = (world, eventEmitter) => {
  ...
  exports.actions = {
      ...
      const getUnitActions = (transition, data, unitActionData) => {
        ...
        return {
          ...
          // TODO
            ...
            model.round_day < controller.configValue("daysOfPeace"), !
            model.battle_isIndirectUnit(e.source.unitId) || exports.notEqual(-
              1, e.movePath.data[0]),
            model.battle_calculateTargets(e.source.unitId, e.target.x,
              e.target.y)
          ]),
      ...
      const getSubMenu = (transition, data, actionData) => {
        ...
        } else if (["propertyTransfer", "unitTransfer"].includes(actionData
            .action)) {
    ...
    handleEndTurn: (world, action) => {
      ...
      exports.client.debug("player " + world.turn.owner.id +
        " ends it turn now")
      ...
      const turnOwnerProperties = world.properties.filter(prop => prop.owner ==
        player)
      ...
      world
        ...
        .map(unit => {
          exports.client.debug("unit " + unit.id +
            " is going to supply neighbours")
          return unit
        })
    ...
    handleYield: (world, actionData) => {
      ...
      exports.client.debug("wiping turn owners properties and units")[world.properties,
          world.units]
    ...
    handleNextCommand(world) {
      ...
      const actionData = {
        ...
        targetUnit: null,
        targetProperty: null,
        targetselection: exports.model.position(commandData.stx,
          commandData.sty)
      }
